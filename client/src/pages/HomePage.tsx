...
export default function HomePage() {
  ...
  return (
    <div className="flex-1 nav-spacer max-w-lg mx-auto w-full overflow-y-auto">
      <div className="px-5 pt-4 pb-3"><UserGreeting user={user} /></div>
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden animate-fade-in">
      ...
      <div className="relative p-6 pb-7">
        ...
        <p className="text-sm text-text-secondary mb-5 max-w-[260px]">Выберите игру и найдите аккаунт за пару минут.</p>
        <button onClick={onBrowse} className="w-full py-3.5 rounded-xl bg-accent text-white font-semibold text-sm active:opacity-80 shadow-lg shadow-accent/30">🎮 Выбрать игру</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-accent/20 glow-border animate-fade-in">
      <div className="bg-accent/10 px-4 py-2 flex items-center justify-between">
        ...
        <span className="flex items-center gap-1.5 text-xs text-success"><span className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" />Активна</span>
      </div>
      <div className="p-4 bg-surface-2">
        <p className="text-xs text-text-muted mb-0.5">{rental.game_name}</p>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-text-muted text-lg font-light mt-[-18px]">:</span>
          <CountdownBlock value={timeLeft.seconds} label="сек" />
        </div>
        <button onClick={onOpen} className="w-full py-3 rounded-xl bg-accent text-white font-semibold text-sm active:opacity-80 shadow-lg shadow-accent/25">Открыть аренду</button>
        <button onClick={onBrowse} className="w-full mt-2 py-2.5 rounded-xl bg-surface-3 text-xs font-medium text-text-secondary active:bg-surface">🎮 Выбрать ещё игру</button>
      </div>
    </div>
  )
}
...
function CountdownBlock({ value, label }: { value: number; label: string }) {
  return <div className="flex-1 text-center"><div className="text-2xl font-bold text-accent tabular-nums">{String(value).padStart(2, '0')}</div><div className="text-[10px] text-text-muted uppercase tracking-wider">{label}</div></div>
}
...
function SellCard({ onStart }: { onStart: () => void }) {
  return (
    <button onClick={onStart} className="w-full text-left rounded-2xl bg-surface-2 border border-white/5 overflow-hidden transition-all active:scale-[0.98] animate-fade-in">
      <div className="flex items-center gap-4 p-4">
        <div className="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center shrink-0"><span className="text-2xl">💰</span></div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5"><p className="text-sm font-bold">Сдать свой аккаунт</p><span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-warning/10 text-warning">Зарабатывайте</span></div>
          <p className="text-xs text-text-muted">Получайте доход, когда не играете.</p>
        </div>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-text-muted shrink-0"><polyline points="9 18 15 12 9 6" /></svg>
      </div>
    </button>
  )
}
