...
export default function AdminUsers() {
  ...
  const handleLevel = async (e: React.MouseEvent, uid: string, level: number) => {
    e.stopPropagation()
  ...
  return (
    <div className="flex-1 nav-spacer max-w-lg mx-auto w-full">
      ...
      <div className="p-4 space-y-3">
        ...
        {loading ? <>{[1,2,3].map(i => <div key={i} className="h-16 skeleton rounded-xl" />)}</> : filtered.length === 0 ? (
          <p className="text-center text-text-muted text-sm py-8">Пользователи не найдены</p>
        ) : filtered.map(u => (
          <button
            key={u.id}
            onClick={() => navigate(`/admin/users/${u.id}`)}
            className="w-full rounded-xl bg-surface-2 border border-white/5 p-3 text-left active:bg-surface-3 transition-colors"
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-full bg-accent/15 flex items-center justify-center text-accent font-bold text-sm shrink-0">{u.first_name[0] || '?'}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">{u.first_name} {u.last_name || ''}</p>
                <p className="text-xs text-text-muted">@{u.username || '—'} · ID: {u.id}</p>
              </div>
              <span className="text-xs text-accent font-medium shrink-0">Lvl {u.level}</span>
            </div>
            <div className="flex gap-1" onClick={e => e.stopPropagation()}>
              {[1,2,3,5,10].map(l => <button key={l} onClick={(e) => handleLevel(e, u.id, l)} className={`px-2 py-1 rounded text-[10px] font-medium ${u.level === l ? 'bg-accent text-white' : 'bg-surface-3 text-text-secondary'}`}>{l}</button>)}
            </div>
          </button>
        ))}
      </div>
